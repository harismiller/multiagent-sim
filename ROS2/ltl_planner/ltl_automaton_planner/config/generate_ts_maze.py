...
def generate_obstacles(grid_size, obstacle_density):
    obstacle_positions = []
    
    for i in range(1, grid_size-1):
        for j in range(1, grid_size-1):
            if (i >= grid_size/2-3 and i <= grid_size/2+3) and (j >= grid_size/2-3 and j <= grid_size/2+3):
                continue
            else:
                # Check for both horizontal and vertical obstacles
                if random.random() < obstacle_density:
                    if random.choice([True, False]):  # True for horizontal, False for vertical
                        # Adding horizontal obstacle
                        obstacle_positions.append([[i, j], [i, j - 1]])
                    else:
                        # Adding vertical obstacle
                        obstacle_positions.append([[i, j], [i - 1, j]])
...
N = 100
obstacle_density = 0.4  # Adjust this value to control the obstacle density
# region = {
...
no_obstacle_list = [
    [[N/2-1, N/2-1],[N/2-2, N/2-1]],
    [[N/2-1, N/2-1],[N/2-1, N/2-2]],
    [[N/2, N/2-1],[N/2, N/2-2]],
...
obstacle_list = generate_obstacles(N, obstacle_density)
obstacle_list = [x for x in obstacle_list if x not in no_obstacle_list]
print(obstacle_list)
data_blocks['blocks'].extend(obstacle_list)

with open('benchmark_block_'+str(N)+'_maze_1.yaml', 'w') as file:
